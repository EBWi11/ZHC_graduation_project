...
<body>
	...
	{
		self.location='add_work.php?id='+id;
	}
	</script>
	...
	{
		self.location='add_services.php?id='+id;
	}
	</script>
<?php
...
while($row = mysqli_fetch_array($result))
  ...
  echo "<td>" . $row['Cword'] . "</td>";
	echo "<td>".'<button name="'. $row['Cid'] .'"  onclick="add_work(this.name)">添加车间信息</button>'." </td>";
	echo "<td>".'<button id="'. $row['Cid'] .'"  onclick="add_services(this.id)">添加后勤信息</button>'." </td>";
  echo "</tr>";
  }
echo "</table>";
mysqli_close($con);
